...
export default function NovelReader() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("pronunciation-dict", false);
        if (res && res.value) setPronunciationDict(JSON.parse(res.value));
  ...
  useEffect(() => {
    ...
    window.storage.set("pronunciation-dict", JSON.stringify(pronunciationDict), false).catch(() => {});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.storage.get(`book-pos:${bookIdRef.current}`, false);
        if (res && res.value) {
  ...
  useEffect(() => {
    ...
    window.storage.set(`book-pos:${bookIdRef.current}`, JSON.stringify({ index: activeIndex }), false).catch(() => {});
  }, [activeIndex, stage]);
